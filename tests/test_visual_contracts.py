...
def test_seed_visual_files_exist_in_public_asset_directory() -> None:
    ...
    for component in build_seed_catalog().values():
        assert component.visual is not None
        for relative_path in (component.visual.file, component.visual.cw_file, component.visual.ccw_file, component.visual.thumbnail):
...
def test_component_without_visual_remains_valid_for_unit_test_fixtures() -> None:
    component = Component(id=999, name="Fixture Motor", type="motor", mass_kg=0.1, parameters_json={"kv": 500.0, "profiles": [{"battery_voltage_v": 22.2, "propeller_id": 30, "points": [{"throttle": 0.0, "thrust_n": 0.0, "current_a": 0.0, "power_w": 0.0}, {"throttle": 1.0, "thrust_n": 10.0, "current_a": 10.0, "power_w": 222.0}]}]})
    assert component.visual is None


def test_legacy_replay_component_receives_visual_without_changing_engineering_data() -> None:
    legacy = Component(id=10, name="Historical Motor", type="motor", mass_kg=0.19, parameters_json={"kv": 360.0, "profiles": [{"battery_voltage_v": 22.2, "propeller_id": 30, "points": [{"throttle": 0.0, "thrust_n": 0.0, "current_a": 0.0, "power_w": 0.0}, {"throttle": 1.0, "thrust_n": 20.0, "current_a": 20.0, "power_w": 444.0}]}]})
