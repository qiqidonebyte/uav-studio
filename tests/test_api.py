...
def test_default_aircraft_contains_real_engineering_and_validation(
    tmp_path: Path,
) -> None:
...
def test_replacing_components_recalculates_and_persists_engineering_data(
    tmp_path: Path,
) -> None:
...
def test_installing_payload_updates_mass_cg_and_flight_time(tmp_path: Path) -> None:
    with build_client(tmp_path / "payload.db") as client:
        ...
        with_payload = current | {
            "payload_id": 80,
            "payload_position_m": {"x": 0.1, "y": 0.0, "z": -0.12},
        }
...
def test_partial_assembly_returns_backend_blocking_errors_and_no_fake_summary(
    tmp_path: Path,
) -> None:
    ...
    assert "REQUIRED_COMPONENT_MISSING" in {
        issue["code"] for issue in state["validation"]["blocking_errors"]
    }
    assert calculate_response.status_code == 200
    assert calculate_response.json()["engineering"] is None


def test_wrong_component_type_is_rejected_by_engineering_validation(
    tmp_path: Path,
) -> None:
    ...
    state = response.json()
    assert state["engineering"] is None
    assert "COMPONENT_TYPE_MISMATCH" in {
        issue["code"] for issue in state["validation"]["blocking_errors"]
    }
