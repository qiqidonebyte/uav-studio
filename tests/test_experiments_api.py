...
@contextmanager
def build_client(database_path: Path, simulations_dir: Path) -> Iterator[TestClient]:
    with TestClient(create_app(database_url=f"sqlite:///{database_path.as_posix()}", simulations_dir=simulations_dir)) as client:
...
def test_target_position_and_waypoints_are_published_to_snapshot(tmp_path: Path) -> None:
    with build_client(tmp_path / "targets.db", tmp_path / "target-simulations") as client:
        simulation_id = create_started_simulation(client)
        target = client.post(f"/api/simulations/{simulation_id}/target", json={"x": 3.0, "y": -2.0})
        waypoints = client.post(f"/api/simulations/{simulation_id}/waypoints", json={"waypoints": [{"x": 2.0, "y": 1.0, "z": 5.0}, {"x": 4.0, "y": -1.0, "z": 5.0}]})
        outside = client.post(f"/api/simulations/{simulation_id}/target", json={"x": 30.0, "y": 0.0})
...
def test_stop_saves_experiment_record_and_telemetry_json(tmp_path: Path) -> None:
    ...
    with build_client(tmp_path / "experiments.db", simulations_dir) as client:
        simulation_id = create_started_simulation(client)
        client.post(f"/api/simulations/{simulation_id}/takeoff", json={"altitude_m": 4.0})
        client.post(f"/api/simulations/{simulation_id}/target", json={"x": 2.0, "y": 3.0})
        time.sleep(0.4)
    ...
    assert {"t", "position", "velocity", "attitude", "angular_velocity", "center_of_gravity", "motors", "forces", "wind", "power", "armed", "flight_mode"} == set(replay_document["frames"][0])
    assert (simulations_dir / f"{simulation_id}.json").exists()


def test_unknown_experiment_returns_not_found(tmp_path: Path) -> None:
    with build_client(tmp_path / "missing-experiment.db", tmp_path / "missing-simulations") as client:
