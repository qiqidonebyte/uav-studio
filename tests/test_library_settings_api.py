...
def build_test_app() -> tuple[FastAPI, object]:
    ...
    def get_db() -> Iterator[Session]:
        with session_factory() as session:
            yield session
    def get_current_user(session: Session = Depends(get_db)) -> UserRecord:
...
def test_settings_api_persists_and_password_change_validates_current_password() -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            wrong = client.put("/api/user/password", json={"current_password": "bad", "new_password": "654321"})
            assert wrong.status_code == 409
            changed = client.put("/api/user/password", json={"current_password": "123456", "new_password": "654321"})
...
def test_component_library_api_filters_clones_and_edits() -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            update_payload = {"name": "API Clone 5010 Edited", "mass_kg": cloned["mass_kg"], "parameters_json": cloned["parameters_json"], "notes": "API test", "tags": ["测试", "自定义"]}
