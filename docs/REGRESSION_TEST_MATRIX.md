...
本表用于每次覆盖部署前的最小验收。原则：简单、有效、不过度设计；只保留能证明学习过程和系统可靠性的检查。
...
| 遥控调试 | 通道、校准、参数检查和上锁门禁正常 | `debug-rc-parameters.test.ts` |
...
| 页面布局 | 顶部导航和学习指引不遮挡工作区，小屏可滚动 | `learning-shell-regression.test.ts`、构建检查 |
...
5. 用超过 12 个学生任务申请 PX4，确认第 13 个显示排队位置；释放一个槽位后队首自动进入。
