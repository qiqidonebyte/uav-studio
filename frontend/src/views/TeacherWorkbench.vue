  ...
  <div class="teacher-page">
          ...
          <section class="split-grid assignment-layout">
            ...
            <div class="panel create-panel">
              ...
              <div class="requirements">
                <span>默认要求</span><b>故障定位 · 修复验证 · Pre-Arm</b>
              </div>
          ...
          <section class="panel">
            <div class="panel-head runs-head">
              ...
              <div class="filter-row">
                ...
                <select v-model="runStatusFilter" @change="refreshRuns"><option value="">全部状态</option><option value="in_progress">进行中</option><option value="completed">已完成</option></select>
              </div>
            </div>
    ...
    <div v-if="runDetail" class="drawer-backdrop" @click.self="runDetail = null">
      <aside class="run-drawer">
        <header><div><span>TRAINING RUN #{{ runDetail.run.id }}</span><h2>{{ runDetail.run.student_name }} · {{ runDetail.run.scenario_title }}</h2></div><button @click="runDetail = null">×</button></header>
        ...
        <div class="verification-strip"><span :class="{ ok: runDetail.run.prearm_passed }">Pre-Arm {{ runDetail.run.prearm_passed ? '通过' : '未通过' }}</span></div>
        <h3>诊断过程</h3>
...
import type { AdminUserView, AssignmentView, ClassroomView, StudentSummary, TeacherOverview, TrainingRunDetail, TrainingRunView } from '../types/teacher'

type TabKey = 'overview' | 'classes' | 'assignments' | 'runs' | 'students' | 'users'
...
const assignmentForm = reactive({ class_id: 0, scenario_id: '', title: '', description: '', due_at: '' })
...
async function publishAssignment(): Promise<void> {
  ...
  try {
    ...
    const created = await teacherApi.createAssignment({
      ...
      requirements: { diagnosis: true, repair: true, prearm: true },
    })
...
function runStatusText(value: string): string { return value === 'completed' ? '已完成' : value === 'in_progress' ? '进行中' : value }
