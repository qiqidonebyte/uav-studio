<template>
  <div class="register-page">
    <main class="register-card" data-testid="register-form">
      ...
      <p v-if="validationHint && !message" class="field-hint">{{ validationHint }}</p>
      <p v-if="message" class="error">{{ message }}</p>
      <button class="primary" data-testid="register-submit" :disabled="auth.loading" @click="submit">
...
const validationHint = computed(() => {
  if (form.username && !usernamePattern.test(form.username)) {
    return '用户名只能包含字母、数字、下划线或连字符，长度 3-32 位。'
  }
